...
export function RolePickerModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl bg-white dark:bg-dark-card rounded-2xl shadow-2xl border border-apple-gray-200 dark:border-dark-border overflow-hidden animate-fade-in">
        {/* Header */}
        <div className="px-8 pt-8 pb-4 text-center">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-apple-blue/10 mb-4">
            <Sparkles className="w-7 h-7 text-apple-blue" />
          </div>
          <h2 className="text-2xl font-bold text-apple-black dark:text-white mb-2">
          ...
          {PM_ROLES.map((role) => (
            <button
              ...
              className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${
                selected === role
                  ? 'border-apple-blue bg-apple-blue/5 shadow-md'
                  : 'border-apple-gray-200 dark:border-dark-border hover:border-apple-blue/40 dark:hover:border-apple-blue/50'
              }`}
            >
              <div className="w-10 h-10 rounded-lg bg-apple-gray-50 dark:bg-dark-hover text-apple-gray-500 flex items-center justify-center">
                {ROLE_ICONS[role]}
              </div>
              <span className="text-sm font-medium text-apple-black dark:text-white text-center">
                {role}
              </span>
              <span className="text-xs text-apple-gray-400 dark:text-slate-400 text-center leading-tight">
                {ROLE_DESCRIPTIONS[role]}
              </span>
              {selected === role && (
                <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-apple-blue flex items-center justify-center">
        ...
        <div className="px-8 py-6 flex justify-end">
          <button
            ...
            className="px-8 py-3 bg-apple-blue hover:bg-apple-blue-hover disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-colors"
          >
